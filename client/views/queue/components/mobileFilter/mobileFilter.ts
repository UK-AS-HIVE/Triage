...
Template.mobileFilter.helpers({
  ...
  filtering: function() {
    return ((Iron.query.get('status') != null) || (Iron.query.get('tag') != null) || (Session.get('ready') === false)) && ((Session.get('queueName') != null) || Session.get('pseudoQueue'));
  }
});

Template.mobileFilter.events({
  'hide.bs.collapse': function(e, tpl) {
    return tpl.$('span[name=' + e.target.id + ']').removeClass('glyphicon-chevron-up').addClass('glyphicon-chevron-down');
  },
  'show.bs.collapse': function(e, tpl) {
    return tpl.$('span[name=' + e.target.id + ']').removeClass('glyphicon-chevron-down').addClass('glyphicon-chevron-up');
  },
  'keyup input[name=textSearch], click button[name=textSearch]': function(e, tpl) {
    var filter, newFilter, newStatus, newTags, newUsers, ref, ref1, ref2, ref3, statuses, tags, terms, text, users;
    if ((e.type === 'click' || e.keyCode === 13) && tpl.$('input[name=textSearch]').val().trim() !== "") {
